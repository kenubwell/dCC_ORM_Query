...
def problem_one(request):
  ...
  students_gpa_greater_than_three = Student.objects.all().filter(gpa__gt=3).order_by('-gpa')    

  for student in students_gpa_greater_than_three:
...
def problem_two(request):
    ...
    instructors_hired_b4_2010 = Instructor.objects.all().filter(hire_date__year__lt=2010).order_by('hire_date')    

    for instructor in instructors_hired_b4_2010:
        print(f'Full Name: {instructor.first_name} {instructor.last_name}')
        print(f'Hire Date: {instructor.hire_date}\n')

    return complete(request)

#Please Note: There were three instructors hired before 2010 but the below (commented) expected terminal results only show two instructors 
...
def problem_three(request):
    ...
    # course_instructor_with_pk2 = Course.objects.all().get(id=2)    
    instructor = Instructor.objects.get(id=2)  
    courses_with_instructor_pk2 = Course.objects.all().filter(instructor_id=2)
    
    print(f'Instructor Name: {instructor.first_name} {instructor.last_name}')
    print('Courses:')
    
    for course in courses_with_instructor_pk2:
        print(f'-{course.name}')
...
def problem_four(request):
    ...
    print(f'Instructors Count: {total_instructors}')
...
# Every time you execute this function a duplicate student will be created with a different primary key number
def problem_five(request):

    new_student = Student.objects.create(first_name="Ken", last_name="Blackwell", year = 12, gpa = 4.0)
...
#Please note that the 'year" column in the student table is for grade year level e.g. 9, 10, 11, 12.  Not calendar year e.g. 2022
...
def problem_six(request):
    ...
    new_student_query = Student.objects.get(id=11)

    print(f'ID: {new_student_query.id}')
    print(f'Full Name: {new_student_query.first_name} {new_student_query.last_name}')
    print(f'GPA: {new_student_query.gpa}')
...
def problem_seven(request):
    ...
    Student.objects.filter(pk=11).delete() #this will delete the student(row) and ID (primary key) 11 which is the new student
